...
Generated emails will be in format:
  ...
  ...
    ...
    if not jira_url:
        print("Error: Jira URL required. Use --url or set JIRA_URL in .env", file=sys.stderr)
        sys.exit(1)

    jira_email = args.email or os.environ.get("JIRA_EMAIL")
    if not jira_email:
        print("Error: Jira email required. Use --email or set JIRA_EMAIL in .env", file=sys.stderr)
        sys.exit(1)
