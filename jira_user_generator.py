...
class JiraUserGenerator:
    ...
    def create_group(self, group_name: str) -> Optional[Dict]:
        ...
        if existing_group:
            ...
            self.existing_groups.append({
                'name': group_name,
                'groupId': existing_group.get('groupId'),
                'status': 'exists'
            })
    ...
    def generate_all(
        ...
        # Final tally
        self.logger.info(f"\nSummary:")
        self.logger.info(f"  Users:  {len(self.existing_users)} existing, {len(invited_users)} invited, {len(failed_users)} failed")
